...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    if settings.REDIS_ENABLED and REDIS_AVAILABLE and settings.REDIS_URL:
        ...
        if not REDIS_AVAILABLE:
            logger.warning("Redis package is not installed. Using dummy implementations for Redis-dependent features.")
            # ダミー実装を提供
            ...
            logger.warning("Redis URL not configured. Redis-dependent features (Events, Rate Limiting Strategy, Caching) might be disabled or limited.")
    # --- レートリミッター初期化 ---
    # storage_uri は Redis が利用可能な場合のみ設定
    redis_storage_uri = settings.REDIS_URL if app.state.redis and REDIS_AVAILABLE else None
    storage_options = {"decode_responses": True} if redis_storage_uri else {} # Redis使用時のオプション
    
    # REDIS_AVAILABLEがFalseの場合、戦略をredisからfixedwindowに変更
    strategy = settings.RATE_LIMIT_STRATEGY
    if not REDIS_AVAILABLE and strategy == "redis":
        strategy = "fixed-window"
        logger.warning("Changed rate limit strategy from 'redis' to 'fixed-window' due to Redis unavailability")
