...
本書は v0.6 系の root `app/` / `libkoiki/` 前提を含む移植資料です。
現行 KOIKI-FW リポジトリ内での正本は次です。

- framework: `components/libkoiki/`
- reference app backend: `components/koiki_ref_app/`
- SAML backend 実装: `components/koiki_ref_app/src/koiki_ref_app/`
- ASGI entrypoint: `koiki_ref_app.asgi:app`

本文中の `app.*` は、移植先プロジェクトが `app` module を使う場合の例として扱ってください。
現行 KOIKI-FW リポジトリ内の作業では、`koiki_ref_app.*` と `components/koiki_ref_app/` を優先します。
