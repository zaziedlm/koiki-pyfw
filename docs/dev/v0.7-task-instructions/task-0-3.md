...
5. `main` も同じ基点、またはその確定コミットから切り出す前提を記録する
6. `main` と `support/0.6` の分岐点として参照できるよう記録を残す
...
- `main` の切り出し元がどのブランチで、どのコミットかを一意に説明できる
...
- これにより、`main`、`support/0.6`、`v0.6-next-base` が同一基点 `523cb79` を参照する状態を作った
...
- Stage 1 以降の作業ブランチは、`main` を `v0.7` 本線、`support/0.6` を `v0.6.x` 保守線として扱う
- remote default branch はまだ `master` のため、GitHub 側の切替は別操作として扱う
- Stage 1 は、`main` を基準にメタデータと依存整理へ進める
