...
Forbidden skills: koiki-refapp-feature-work, koiki-libkoiki-feature-work
...
Prompt: reference app の Todo API に承認フロー依存の業務ルールを追加してください。既存の libkoiki 共通機能は再利用したいです。

Expected skills: koiki-refapp-feature-work
Required first skill: koiki-refapp-feature-work
Forbidden skills: koiki-project-overview, koiki-libkoiki-feature-work
Rationale: reference app 固有の backend 変更は refapp feature work が第一候補。
...
Forbidden skills: koiki-project-overview, koiki-refapp-feature-work
...
Expected skills: koiki-libkoiki-feature-work
Required first skill: koiki-libkoiki-feature-work
Forbidden skills: koiki-refapp-feature-work
...
Prompt: 特定顧客向けの請求連携 API を downstream の apps/ 配下に実装してください。components/ は編集せず、既存の koiki_ref_app と libkoiki を組み合わせて構成します。

Expected skills: koiki-business-app-feature-work
Required first skill: koiki-business-app-feature-work
Forbidden skills: koiki-refapp-feature-work, koiki-libkoiki-feature-work
Rationale: components/ を編集しない downstream / 顧客固有の apps/ API は business app feature work が第一候補。reference app 用の refapp feature work とは区別する。

Observed skills:
Pass/Fail:
Notes:

## business-app-vs-refapp-disambiguation

Prompt: この新しい在庫管理 API は特定案件専用です。reference app (koiki_ref_app) ではなく downstream の apps/ 配下に置いて実装してください。

Expected skills: koiki-business-app-feature-work
Required first skill: koiki-business-app-feature-work
Forbidden skills: koiki-refapp-feature-work
Rationale: 案件専用 API は apps/ の business app feature work。reference app 用の refapp feature work とは1文字違いで混同しやすいため明確に区別する。
...
Forbidden skills: koiki-project-overview, koiki-refapp-feature-work, koiki-libkoiki-feature-work
...
Prompt: reference app の SAML SSO ログインフローを修正してください。必要なら app 層と libkoiki 層の両方を見てください。

Expected skills: koiki-auth-security, koiki-refapp-feature-work
Required first skill: koiki-auth-security
Forbidden skills: (none)
Rationale: SSO/SAML は auth security を含みつつ、reference app 固有実装なら refapp feature も必要。
