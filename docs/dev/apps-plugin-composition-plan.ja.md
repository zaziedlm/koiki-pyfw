...
- 業務アプリ固有 model の Base 定義は `apps/db/base.py` で所有する
...
業務アプリ固有 model は、`components/koiki_ref_app.db.base.Base` を直接前提にし続けるのではなく、`apps/db/base.py` に業務アプリ固有の Base を用意する。
...
- 業務アプリ固有 model は `apps/db/base.py` の `Base` を import する方針が説明されている
