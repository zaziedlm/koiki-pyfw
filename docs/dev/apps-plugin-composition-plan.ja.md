...
`apps/` 配下を downstream / 業務アプリチームの backend API 編集領域として使えるようにしつつ、`components/` 配下を業務アプリ固有 API のために直接改修しなくてよい構造を定義する。
...
一方で、業務アプリ固有の API を `apps/` 配下に置く場合、どこかで FastAPI の `include_router()` を呼び出す必要がある。
...
- `components/` は `apps/` を import しない
- 業務アプリ固有 router / service / model / plugin 登録は `apps/` に閉じる
- 業務アプリ固有 model の Base は、既定では `koiki_ref_app.db.base.Base` を利用する。業務固有の主キー・tenant_id・監査カラムなど独自設計が必要な場合のみ `apps/db/base.py` で独自 Base を定義する
- 業務アプリの ASGI entrypoint は `apps/asgi.py` で所有する
...
この形なら、業務アプリチームは `apps/` だけを編集して API を追加できる。

### 3. 業務アプリの DB Base 方針

業務アプリ固有 model の Base は **既定では `koiki_ref_app.db.base.Base`** を利用する。

```python
# apps/models/order.py（既定パターン）
...
この Base は `libkoiki.db.base.Base` と registry / metadata を共有しており、migration target に自動で乗る。`components/` を改修せずに利用できる。

**例外：独自 Base が必要な場合**

業務固有の主キー型・`tenant_id`・監査カラム・命名規則など、reference app 共通カラム設計と異なる要件がある場合のみ `apps/db/base.py` で独自 Base を定義する。

```python
# apps/db/base.py（独自 Base が必要な場合のみ）
...
この形なら `koiki_ref_app` 向け共通カラム設計に縛られず、`tenant_id`・監査カラム・業務固有 ID を apps 側で設計できる。ただし、`apps/db/base.py` を用意するかどうかは案件要件で判断し、既定では不要。
...
- `apps/asgi.py` が業務アプリ composition の所有場所であると説明されている
...
uv run --locked python -c "from apps.asgi import app; print(app.title)"
```
...
- reference app と apps 側 business app composition の両方が import できる
...
- downstream が ASGI target を `apps.asgi:app` に差し替える方法が説明されている
...
- 業務アプリ固有 model の Base は既定で `koiki_ref_app.db.base.Base` を利用する方針が説明されている
- `apps/db/base.py` での独自 Base 定義は、業務固有の主キー・カラム設計が必要な場合のみと明記されている
...
その根拠は、業務アプリの composition を root `main.py` ではなく `apps/asgi.py` が所有する、という本設計タスクである。
