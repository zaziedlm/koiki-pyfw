---
name: koiki-project-overview
description: Classify a KOIKI task and choose the correct repository layer when the edit location is not yet clear.
---

# KOIKI Project Overview

Use the canonical project overview guidance:

@docs/agent/skills/koiki-project-overview/SKILL.md

Always apply these shared rules:

- @docs/agent/boundaries.md
- @docs/agent/architecture.md
- @docs/agent/testing.md

If the task becomes clear, continue with the more specific KOIKI skill for that layer.
