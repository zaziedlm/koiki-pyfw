...
class LoginSecurityService:
    ...
    async def check_login_allowed(
        ...
        """
        # セッションを設定
        self.login_attempt_repository.session = db
        
        # メールアドレス単位のチェック
        email_attempts = await self.login_attempt_repository.count_recent_failed_attempts_by_email(
            email, self.lockout_duration_minutes
        )
        
        if email_attempts >= self.max_attempts_per_email:
            return False, f"Account temporarily locked due to {email_attempts} failed attempts", self.lockout_duration_minutes * 60
        
        # IP単位のチェック
        ip_attempts = await self.login_attempt_repository.count_recent_failed_attempts_by_ip(
            ip_address, self.lockout_duration_minutes
        )
        
        if ip_attempts >= self.max_attempts_per_ip:
            return False, f"IP temporarily blocked due to {ip_attempts} failed attempts", self.lockout_duration_minutes * 60
