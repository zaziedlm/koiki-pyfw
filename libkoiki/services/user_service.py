...
from libkoiki.core.exceptions import ValidationException, ResourceNotFoundException, AuthenticationException, AuthorizationException
...
class UserService:
    ...
    async def get_user_by_id(self, user_id: int, db: AsyncSession) -> Optional[UserModel]: # Optionalを返すように修正
        """IDでユーザーを取得します"""
    ...
    async def get_users(self, skip: int, limit: int, db: AsyncSession) -> Sequence[UserModel]:
        """ユーザーリストを取得します"""
    ...
    async def create_user(self, user_in: UserCreate, db: AsyncSession) -> UserModel:
        ...
        if not check_password_complexity(user_in.password):
            logger.warning("Service: Password does not meet complexity requirements", email=user_in.email)
        ...
        # ORMモデルインスタンスを作成
        user_data = user_in.dict(exclude={"password"}) # スキーマから辞書へ (パスワード除外)
        ...
        if self.event_publisher:
            try:
                await self.event_publisher.publish(
                    "user_created", {"user_id": created_user.id, "email": created_user.email}
                )
    ...
    async def update_user(self, user_id: int, user_in: UserUpdate, db: AsyncSession) -> UserModel:
        """ユーザー情報を更新します。"""
        logger.info("Service: Updating user", user_id=user_id, data=user_in.dict(exclude_unset=True))
        self.repository.set_session(db)
        ...
        if "password" in update_data and update_data["password"]:
            ...
            if not check_password_complexity(update_data["password"]):
                 logger.warning("Service: New password does not meet complexity requirements", user_id=user_id)
                 raise ValidationException(
        ...
        if "email" in update_data and update_data["email"] != user.email:
             logger.debug("Service: Checking for email duplication on update", new_email=update_data["email"])
             existing_user = await self.repository.get_by_email(update_data["email"])
             if existing_user and existing_user.id != user_id:
                 logger.warning("Service: Email already exists during update", new_email=update_data["email"])
                 raise ValidationException("This email address is already registered by another user.")
    ...
    async def authenticate_user(self, email: str, password: str, db: AsyncSession) -> Optional[UserModel]:
        ...
        if not verify_password(password, user.hashed_password):
            logger.info("Authentication failed: Incorrect password", email=email, user_id=user.id)
    ...
    async def delete_user(self, user_id: int, db: AsyncSession) -> Optional[UserModel]:
        ...
        if self.event_publisher:
             try:
                 ...
                 await self.event_publisher.publish("user_deleted", {"user_id": deleted_user.id, "email": deleted_user.email})
             except Exception as e:
