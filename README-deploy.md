...
## 2.1. ローカル開発（コンテナ外でアプリケーション実行） (2025-07-10 追加)

Dockerコンテナ内ではなく、ローカル環境でアプリケーションを直接実行したい場合：
...
# "database system is ready to accept connections" が表示されるまで待機
```

```powershell
# 3. ローカル開発用の環境変数を設定（PowerShell）
...
毎回環境変数を設定する手間を省くため、ローカル開発用の環境変数ファイルを作成することをお勧めします：
...
その後、以下のコマンドでアプリケーションを起動：

```powershell
# 環境変数ファイルを読み込んで起動
Get-Content .env.local | ForEach-Object { if ($_ -match '^([^=]+)=(.*)$') { [System.Environment]::SetEnvironmentVariable($matches[1], $matches[2]) } }
uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload
```

### 自動化スクリプト使用

より簡単にローカル開発を開始するため、PowerShellスクリプトを使用できます：
...
`start-local-dev.ps1`スクリプトは以下の処理を自動実行します：
1. データベースコンテナの起動と健全性確認
2. 環境変数の自動設定
3. データベースマイグレーションの実行
4. アプリケーションの起動

### 注意点
- この方法ではデータベースのみDockerコンテナを使用
- アプリケーションはローカル環境のPython仮想環境で実行
- データベース接続は`localhost:5432`経由
- コードの変更が即座に反映される（--reload オプション）
