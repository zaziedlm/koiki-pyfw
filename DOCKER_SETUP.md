...
- 初回のみ `.env` と `frontend/.env.local` をテンプレから作成
  ...
  ```
- production profile を使う場合のみ、production 用テンプレートも作成
  ```powershell
...
- `.env.production`: backend の prod / prod-external 用（`ENV_FILE=.env.production` としてセットされる）
- `frontend/.env.production`: frontend の prod / prod-external 用（`FRONTEND_ENV_FILE=./frontend/.env.production` としてセットされる）
