...
class SecurityAPITester:
    ...
    def login(self, email: str, password: str) -> Optional[str]:
        """ログインしてトークンを取得 (OAuth2PasswordRequestForm形式)"""
        # OAuth2PasswordRequestFormはform-urlencoded形式を要求
        form_data = f"username={urllib.parse.quote(email)}&password={urllib.parse.quote(password)}"

        try:
            response = self.make_request(
                "POST", f"{self.base_url}/api/v1/auth/login", form_data=form_data
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ ログイン失敗: {email} - {response.status_code}: {response.text}"
                )

        except Exception as e:
            print(f"❌ ログインエラー: {email} - {e}")
