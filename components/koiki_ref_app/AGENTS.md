...
- application-level composition of `components/libkoiki/` behavior

Before changing code in this directory:

- confirm the behavior is specific to this application
- reuse `components/libkoiki/` capabilities before adding new framework logic
- follow the existing `src/koiki_ref_app/api/v1/` and `src/koiki_ref_app/main.py` structure
