...
class Settings(BaseSettings):
    ...
    REDIS_ENABLED: bool = False  # Redisを使用するか
    ...
    # React browser-session compatibility layer.
    # Access/refresh tokens are HttpOnly cookies; the CSRF token is a separate
    # readable double-submit cookie. Production always forces Secure cookies.
    BROWSER_ACCESS_TOKEN_COOKIE_NAME: str = "koiki_access_token"
    BROWSER_REFRESH_TOKEN_COOKIE_NAME: str = "koiki_refresh_token"
    BROWSER_CSRF_COOKIE_NAME: str = "koiki_csrf_token"
    BROWSER_CSRF_HEADER_NAME: str = "x-csrf-token"
    BROWSER_COOKIE_SAMESITE: str = "lax"
    BROWSER_COOKIE_SECURE: bool = False
    BROWSER_CSRF_TOKEN_MAX_AGE_SECONDS: int = 24 * 60 * 60
